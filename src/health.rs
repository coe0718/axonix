...
fn parse_docker_containers(json: &str) -> DockerHealth {
    ...
    for entry in trimmed.split("\"Names\"") {
        ...
        // Find the first quoted string inside the array after ":"
...
/// e.g. `extract_json_string_value(fragment, "\"State\"")` → "running"
...
mod tests {
    ...
    fn test_docker_health_no_panic() {
        // docker_health() may fail (Docker not available in test env), but must not panic.
        // It should return DockerHealth with error set gracefully.
        let result = docker_health();
        // Either succeeded (in Docker env) or failed gracefully
