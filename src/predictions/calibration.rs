//! Calibration logic for `PredictionStore`.
//!
//! Extracted from `store.rs` to keep that file under 300 lines (Issue #110).
//! These methods are implemented on `PredictionStore` as an extension in this file.
...
impl PredictionStore {
    ...
    pub fn auto_resolve_from_goals(&mut self, goals_archive_content: &str) -> Vec<(u32, String)> {
        let open_list: Vec<(u32, super::types::Prediction)> = self
            .open()
            .into_iter()
            .map(|(id, p)| (id, p.clone()))
            .collect();
