//! Handles `/status`, `/context`, and `/tokens` inline REPL commands.
//!
//! These commands need live session data (agent messages, session elapsed time,
//! token counts) but do not need async I/O. Separated from cmd_dispatch.rs to
//! keep each file ≤ 300 lines (G-148).
...
/// Handle inline session-state commands that are not dispatched by `handle_command()`.
///
